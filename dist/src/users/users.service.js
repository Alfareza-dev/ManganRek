...
let UsersService = class UsersService {
    ...
    async createCashier(adminId, dto) {
        const adminResto = await this.prisma.restaurant.findUnique({
            where: { ownerId: adminId },
        });
        if (!adminResto) {
            throw new common_1.NotFoundException('Restoran tidak ditemukan untuk admin ini');
        }
    ...
    async updateCashierPassword(adminId, cashierId, newPassword) {
        const adminResto = await this.prisma.restaurant.findUnique({
            where: { ownerId: adminId },
        });
        if (!adminResto)
